[CmdletBinding()]
param(
    [string]$Destination = (Join-Path $PSScriptRoot '..\release\PMDO-Android-Player-Thor-Update-v0.1.8.zip'),
    [switch]$Force
)
...
if ($displayVersion -cne '0.1.8' -or $versionCode -cne '9') {
    throw "Thor update requires Android 0.1.8/versionCode 9; got $displayVersion/$versionCode."
}
